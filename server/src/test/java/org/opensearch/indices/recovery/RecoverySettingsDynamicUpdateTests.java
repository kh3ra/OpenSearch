...
public class RecoverySettingsDynamicUpdateTests extends OpenSearchTestCase {
    ...
    public void testZeroBytesPerSecondIsNoRateLimit() {
        ...
        clusterSettings.applySettings(
            Settings.builder().put(RecoverySettings.INDICES_REPLICATION_MERGED_SEGMENT_WARMER_MAX_BYTES_PER_SEC_SETTING.getKey(), 0).build()
        );
    ...
    public void testSetMergedSegmentReplicationMaxBytesPerSec() {
        ...
        clusterSettings.applySettings(
            Settings.builder()
                .put(
                    RecoverySettings.INDICES_REPLICATION_MERGED_SEGMENT_WARMER_MAX_BYTES_PER_SEC_SETTING.getKey(),
    ...
    public void testMergedSegmentReplicationTimeout() {
        ...
        clusterSettings.applySettings(
            Settings.builder()
                .put(RecoverySettings.INDICES_REPLICATION_MERGED_SEGMENT_WARMER_TIMEOUT_SETTING.getKey(), TimeValue.timeValueMinutes(5))
                .build()
        );
        assertEquals(5, (int) recoverySettings.getMergedSegmentReplicationTimeout().minutes());
        clusterSettings.applySettings(
            Settings.builder()
                .put(RecoverySettings.INDICES_REPLICATION_MERGED_SEGMENT_WARMER_TIMEOUT_SETTING.getKey(), TimeValue.timeValueMinutes(25))
                .build()
        );
    ...
    public void testMergedSegmentReplicationWarmerEnabledSetting() {
        ...
        clusterSettings.applySettings(
            Settings.builder().put(RecoverySettings.INDICES_REPLICATION_MERGED_SEGMENT_WARMER_ENABLED_SETTING.getKey(), true).build()
        );
        assertTrue(recoverySettings.isMergedSegmentReplicationWarmerEnabled());

        clusterSettings.applySettings(
            Settings.builder().put(RecoverySettings.INDICES_REPLICATION_MERGED_SEGMENT_WARMER_ENABLED_SETTING.getKey(), false).build()
        );
    ...
    public void testMergedSegmentWarmerSegmentSizeThresholdSetting() {
        ...
        clusterSettings.applySettings(
            Settings.builder()
                .put(RecoverySettings.INDICES_REPLICATION_MERGED_SEGMENT_WARMER_SEGMENT_SIZE_THRESHOLD_SETTING.getKey(), "100gb")
                .build()
        );
        ...
        clusterSettings.applySettings(
            Settings.builder()
                .put(RecoverySettings.INDICES_REPLICATION_MERGED_SEGMENT_WARMER_SEGMENT_SIZE_THRESHOLD_SETTING.getKey(), "4KB")
                .build()
        );
        assertEquals(4L, recoverySettings.getMergedSegmentWarmerSegmentSizeThreshold().getKb());
    }

    public void testMergeBasedIndexingThrottleThresholdMultiplier() {
        FeatureFlags.initializeFeatureFlags(Settings.builder().put(FeatureFlags.MERGED_SEGMENT_WARMER_EXPERIMENTAL_FLAG, true).build());

        assertEquals(1.0, recoverySettings.getIndexingThrottleInflightMergesThreshold(), 0);

        clusterSettings.applySettings(
            Settings.builder()
                .put(RecoverySettings.INDICES_INDEXING_THROTTLE_INFLIGHT_MERGES_THRESHOLD_SETTING.getKey(), "5")
                .build()
        );
        assertEquals(5.0, recoverySettings.getIndexingThrottleInflightMergesThreshold(), 0);

        clusterSettings.applySettings(
            Settings.builder()
                .put(RecoverySettings.INDICES_INDEXING_THROTTLE_INFLIGHT_MERGES_THRESHOLD_SETTING.getKey(), "0")
                .build()
        );
        assertEquals(0.0, recoverySettings.getIndexingThrottleInflightMergesThreshold(), 0);
    }
}
